...
That helper needs only local `ssh`. It does not depend on your personal `~/.ssh/known_hosts`, survives router factory resets, and runs the actual bootstrap on the router. For the normal path you do not need `ssh-keygen`, local `wget`, or local `curl`.
