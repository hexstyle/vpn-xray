...
| --- | --- | --- |
| `routers/*/files/xray.html` | ~4.0k → 527 | **CSS/JS extracted + display cells DRY'd, browser-verified.** `<style>` → `xray-base.css` + `xray-components.css`; `<script>` → `xray-app-1…7.js` (deployed to `/www/`, referenced in original load order; byte-identical split, each JS chunk `node --check`-clean). Then the 7 near-identical Live-State cells and 3 Detailed-Compare tables were replaced by `STATUS_CELLS`/`COMPARE_CELLS` data + a small render in `xray-app-1.js` (genuine DRY, −47 lines). Playwright-verified both times: 0 console/page errors, all elements + classes present, render identical to baseline. **Residual (527): the rest is declarative FORM markup (35 fields) + distinct sections with no more clean repeats. Data-driving forms would bury labels/input-types/a11y attrs in JS — a real regression — so 527 is the genuine-improvement optimum; the last 27 lines to hit 500 are deliberately NOT forced.** |
| ~~`routers/common/files/router-rules`~~ | ~~3.8k~~ → 229 | **DONE.** Ten `router-rules-*.sh` libs (config, git, repo, remote, rulestree, external-a, external-b, ipset, apply, status) in `common/files/`, deployed to `/usr/share/vpn-xray/`, sourced via `$VX_RULES_LIB_DIR` (tests override it to the repo). Header + `with_lock`/`usage` + LIB_ONLY guard + subcommand dispatch stay in the main script |
...
| ~~`bootstrap-router-vps.sh`~~ | ~~1.0k~~ → 314 | **DONE.** Function-heavy (670 of 981 lines were function bodies). All 40 top-level function definitions lifted, heredoc-aware and byte-identical (full reconstruction verified), into `common/lib/bootstrap-lib-a.sh` + `bootstrap-lib-b.sh`, sourced right after `common/lib/env.sh`. The ~311-line top-level orchestration stays inline and runs in its original order (only definitions moved, so no use-before-def is possible). Verified: `bash -n` + `--help` renders the full usage |
...
| ~~`routers/*/install-router.sh`~~ | ~~0.8k~~ → 345 | **DONE.** Per-profile `install-router-lib.sh` (sourced via `$(dirname "$BASH_SOURCE")`) holds the 13 helper functions plus `install_router_main()`, which wraps the entire post-preflight deploy/verify tail verbatim. Behaviour is provably identical: reconstruction (inline the funcs + tail body back into the main) is asserted byte-for-byte against the original, and the wrapped tail was scanned to contain no positional params / `return` / `local` / `declare` / `shift` / `getopts` (the only ways a bash function wrapper could change semantics), so it runs identically in one call. `install_router_main` is called at end-of-main, after the `--preflight` early-exit. Verified: `bash -n` + `--preflight` (front half + lib sourcing) on the live router |
| `vps/debian-13/files/install-vps.remote.sh` | ~0.6k | **Deferred (delivery-locked).** Function-heavy (17 step fns + a small tail), so it *would* split cleanly — but it is rendered (placeholder substitution) and delivered to the VPS as ONE self-contained file via `cat >`/tar, then run with `sh`. Splitting the source into two <500 files would require both render paths (`install-vps.sh` `render_template` AND `xray-vps-render.sh` `render_vps_profile_template` — the latter is the sensitive one that once corrupted /root) to concatenate the two parts, and it cannot be verified without a real VPS provision. Not worth that risk on the provisioning path for ~100 lines; revisit only if it grows materially |
