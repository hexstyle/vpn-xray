...
| --- | --- | --- |
| `routers/*/files/xray.html` | ~4.0k | **Deferred (needs browser verification + restructure).** Blocks: `<style>` ~662 lines, `<script>` ~2796 lines, and the HTML body markup ~552 lines. Extracting CSS → `xray.css` (split in two, each <500) and JS → `xray-*.js` modules is the plan, but the body markup alone still exceeds 500, so getting fully under the cap needs HTML templating/partials — a behaviour-changing restructure whose only real test is a browser. Do this from a browser-capable session; do NOT ship it verified only by string-grep tests |
