...
| --- | --- | --- |
| `routers/*/files/xray.html` | ~4.0k → 574 | **CSS/JS extracted, browser-verified.** `<style>` → `xray-base.css` + `xray-components.css`; `<script>` → `xray-app-1…7.js` (all in `common/files/`, deployed to `/www/`, referenced via `<link>`/`<script src>` in original load order). Split is byte-identical (chunk concat == original) and each JS chunk passes `node --check`; verified live with Playwright — 0 console/page errors, all 9 assets load, identical render vs the pre-split baseline. **Residual: the 574-line file is now pure declarative body markup**; dropping under 500 would need HTML-in-JS templating (behaviour-changing, worse maintainability) — left as-is |
