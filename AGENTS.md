...
| `routers/*/install-router.sh` | ~0.8k | **Deferred (top-level-heavy).** Unlike bootstrap, only 154 of ~785 lines are function bodies (identical across profiles); the other ~631 are the orchestration flow itself. Lifting the functions leaves the file at ~635 — still over 500. Getting under the cap needs wrapping big top-level blocks into step functions (behaviour-changing, not byte-identical) on the critical install path, which cannot be dry-run without a real router+VPS install. Shrink when this file is next edited with live install coverage |
