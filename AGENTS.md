...
| `bootstrap-router-vps.sh` | ~1.0k | **Deferred (shrink-on-touch).** Procedural orchestrator — ~190 top-level statements interleaved with functions, so there is no clean function block to lift; a safe split needs wrapping the flow in a `main()` (behaviour-changing, not byte-identical). Do it when this file is next edited for another reason |
...
| `routers/*/install-router.sh` | ~0.8k | **Deferred (shrink-on-touch).** Same shape as bootstrap — heavily interleaved top-level orchestration plus a ~170-line profile-specific remote-probe function; not safely byte-identical-splittable. Shrink when next touched |
