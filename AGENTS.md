...
| `vps/debian-13/files/install-vps.remote.sh` | ~0.6k | **Deferred (not lib-splittable).** Rendered and piped to the VPS as one self-contained script — there are no sibling files on the VPS to source, so the sourced-lib pattern does not apply. Only ~100 lines over; revisit only if it grows materially |
